...
public class Mensagem {
	...
	private List<MudancaEstadoObserver> observers;
	...
	public Mensagem(String conteudo) {
		...
		this.observers = new ArrayList<MudancaEstadoObserver>();
	}
	...
	public void assine() throws OperationNotSupportedException{
		MensagemState anterior = this.state;
		this.state = this.state.assinar();
		if(anterior != this.state)
			this.notifyMudancaEstado(anterior, this.state);
	...
	public void attachMudancaEstadoObserver(MudancaEstadoObserver observer) {
		this.observers.add(observer);
	}
	
	public void detachMudancaEstadoObserver(MudancaEstadoObserver observer) {
		this.observers.remove(observer);
	}
	
	private void notifyMudancaEstado(MensagemState velho, MensagemState novo) {
		for(MudancaEstadoObserver observer : this.observers)
			observer.notifyMudancaEstado(new EventoMudancaEstado(this.conteudo, velho.desc(), novo.desc()));
	}
	
	public MensagemState getMensagemState() {
		return this.state;
	}
